...
def load_data_and_train():
    ...
    # Konversi Kecepatan ke Numerik DULU agar tidak bentrok dengan loop object
    ...
    # Loop hanya untuk kolom kategori asli (yang bertipe object)
...
try:
    ...
    for i, col in enumerate(selected_features):
        ...
        if col == 'kecepatan':
            val_min = int(df[col].min())
            val_max = int(df[col].max())
            val_default = int(df[col].median())
            if val_min == val_max: val_max = val_min + 1
            input_data[col] = target_col.slider(label, val_min, val_max, val_default)
        else:
            # Pastikan semua opsi adalah string untuk menghindari error sorting
