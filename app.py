...
def load_data_and_train():
    try:
        ...
        df['kecepatan'] = df['kecepatan'].fillna(df['kecepatan'].median()).astype(int)
        ...
        # Model A: SMOTETomek + RF
        smt = SMOTETomek(random_state=42)
        X_train_res, y_train_res = smt.fit_resample(X_train, y_train)
        m_smote = RandomForestClassifier(n_estimators=500, max_depth=12, min_samples_leaf=2, random_state=42, n_jobs=-1)
        m_smote.fit(X_train_res, y_train_res)

        # Model B: Balanced RF
        m_balanced = BalancedRandomForestClassifier(n_estimators=300, max_depth=10, min_samples_leaf=2, sampling_strategy='all', random_state=42, n_jobs=-1)
...
if df is not None:
    ...
    if st.button("MULAI ANALISIS RISIKO"):
        ...
        classes = list(model_balanced.classes_)
        fatal_idx = classes.index('kecelakaan fatal')
        ...
        if probs_balanced[fatal_idx] >= 0.37:
            ...
            others = [i for i in range(len(classes)) if i != fatal_idx]
        ...
        res = dict(zip(classes, final_probs))
        
        st.write("### Visualisasi Risiko:")
        fig, ax = plt.subplots(figsize=(8, 8))
        colors = ['#f39c12', '#e74c3c', '#27ae60']
        order = ['kecelakaan berat', 'kecelakaan fatal', 'kecelakaan ringan']
        vals = [res.get(k, 0) for k in order]
        
        ax.pie(vals, labels=[k.upper() for k in order], autopct='%1.1f%%', startangle=140, 
               colors=colors, explode=(0.05, 0.05, 0.05), shadow=True, textprops={'fontweight': 'bold'})
        plt.title('Risiko Tingkat Keparahan LAKA', fontsize=16, fontweight='bold')
        st.pyplot(fig)

        st.write("### Rincian Probabilitas:")
        st.json(res)
