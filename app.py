...
def load_data_and_train():
    ...
    # Split data persis Colab
    X_train, _, y_train, _ = train_test_split(X_all, y_all, test_size=0.2, stratify=y_all, random_state=42)

    # Resampling SMOTETomek persis Colab
    smt = SMOTETomek(random_state=42)
    X_train_res, y_train_res = smt.fit_resample(X_train, y_train)

    # Model SMOTE - RandomForest (Eksplisit max_features='sqrt' untuk sinkronisasi)
    m_smote = RandomForestClassifier(n_estimators=500, max_depth=12, min_samples_leaf=2, max_features='sqrt', bootstrap=True, class_weight='balanced', random_state=42)
    m_smote.fit(X_train_res, y_train_res)

    # Model Balanced RF (Eksplisit parameter untuk sinkronisasi)
    m_balanced = BalancedRandomForestClassifier(n_estimators=300, max_depth=10, min_samples_leaf=2, max_features='sqrt', sampling_strategy='all', replacement=False, random_state=42)
...
try:
    ...
    if st.button("MULAI ANALISIS RISIKO"):
        ...
        # Logika Prediksi Hybrid Threshold 37% persis Colab
