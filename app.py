...
def load_data_and_train():
    ...
    # Perbaikan Konversi Kecepatan secara total untuk menghindari ValueError
...
try:
    ...
    for i, col in enumerate(selected_features):
        ...
        if df[col].dtype == 'object':
            ...
            clean_col = df[col].fillna(df[col].median() if not df[col].isna().all() else 0)
            min_val = int(clean_col.min())
            max_val = int(clean_col.max())
            default_val = int(clean_col.median())
