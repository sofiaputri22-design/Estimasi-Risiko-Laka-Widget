...
def load_data_and_train():
    # Memastikan file dataset terbaca
    ...
    # Perbaikan Konversi Kecepatan secara total untuk menghindari ValueError dan Median Error
    ...
    # Model B: Balanced Random Forest
...
try:
    ...
    st.markdown("<div class='header-box'><h1>Accident Risk Predictor</h1><p>Kabupaten Gresik - Corporate Yellow Theme</p></div>", unsafe_allow_html=True)
    ...
    for i, col in enumerate(selected_features):
        ...
        if df[col].dtype == 'object':
            ...
            # Proteksi ekstra terhadap NaN pada kolom numerik saat runtime
            # Gunakan pd.to_numeric untuk memastikan tidak ada string yang lolos ke median()
            clean_col = pd.to_numeric(df[col], errors='coerce')
            col_median = clean_col.median() if not clean_col.isna().all() else 0
            clean_col = clean_col.fillna(col_median)
            
            min_val = int(clean_col.min())
            max_val = int(clean_col.max())
            default_val = int(col_median)

            # Pastikan min tidak sama dengan max untuk slider
            if min_val == max_val:
                max_val = min_val + 1

            input_data[col] = target_col.slider(label, min_val, max_val, default_val)
    ...
    if st.button("MULAI ANALISIS RISIKO"):
        ...
        st.markdown("<div class='result-card'><h2>Hasil Estimasi Keparahan</h2></div>", unsafe_allow_html=True)
        c1, c2 = st.columns([1.5, 1])
        with c1:
            fig, ax = plt.subplots()
            ax.pie(res.values(), labels=[k.upper() for k in res.keys()], autopct='%1.1f%%', colors=['#f39c12', '#e74c3c', '#27ae60'])
            st.pyplot(fig)
        with c2:
    ...
    st.error("Dataset 'Dataset Klasifikasi - Copy.csv' tidak ditemukan. Pastikan file ini ada di folder yang sama dengan app.py.")
